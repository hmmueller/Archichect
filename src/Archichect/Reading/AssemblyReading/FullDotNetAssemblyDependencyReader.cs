...
namespace Archichect.Reading.AssemblyReading {
    public class FullDotNetAssemblyDependencyReader : AbstractDotNetAssemblyDependencyReader {
        ...
        private static readonly MarkerGenerator<TypeDefinition>[] _typeDefinitionMarkers = {
            ...
            CreateCheck<TypeDefinition>(t => t.IsRuntimeSpecialName, _runtimespecialname),
            CreateCheck<TypeDefinition>(t => t.IsSpecialName, _specialname),
            // and 1000s more
        };
        ...
        private static readonly MarkerGenerator<MethodDefinition>[] _methodDefinitionMarkers = {
            ...
            CreateCheck<MethodDefinition>(t => t.IsFamilyOrAssembly || t.IsFamily || t.IsFamilyAndAssembly, _protected),
            CreateCheck<MethodDefinition>(t => t.IsCompilerControlled ||t.IsFamilyOrAssembly || t.IsAssembly|| t.IsFamilyAndAssembly, _internal),
        ...
        private IEnumerable<RawDependency> AnalyzeProperty([NotNull] TypeDefinition owner, [NotNull] RawUsingItem usingItem, [NotNull] PropertyDefinition property, [CanBeNull] ItemTail typeCustomSections, WorkingGraph readingGraph) {
            ...
            foreach (var dependency in AnalyzeCustomAttributes(usingItem, property.CustomAttributes, readingGraph)) {
                yield return dependency;
            }

            foreach (ParameterDefinition parameter in property.Parameters) {
                foreach (var dependency in CreateTypeAndMethodDependencies(usingItem, DOTNETPARAMETER,
        ...
        private IEnumerable<RawDependency> AnalyzeCustomAttributes([NotNull] RawUsingItem usingItem,
            ...
            foreach (CustomAttribute customAttribute in customAttributes) {
                ...
                //foreach (var dependency in CreateTypeAndMethodDependencies(usingItem, null, customAttribute.Constructor.DeclaringType,
        ...
        private IEnumerable<RawDependency> AnalyzeMethod([NotNull] TypeDefinition owner, [NotNull] RawUsingItem usingItem,
            ...
            foreach (var dependency in AnalyzeCustomAttributes(usingItem, method.CustomAttributes, readingGraph)) {
                yield return dependency;
            }
            ...
            if (method.HasBody) {
                foreach (var variable in method.Body.Variables) {
                    if (!IsLinked(variable.VariableType, method.DeclaringType.DeclaringType)) {
                        foreach (var dependency in CreateTypeAndMethodDependencies(usingItem, DOTNETVARIABLE,
                ...
                foreach (Instruction instruction in method.Body.Instructions) {
                    ...
                    foreach (var dependency in AnalyzeInstruction(owner, usingItem, instruction, mostRecentSeqPoint, readingGraph)) {
                        yield return dependency;
                    }
                }
            }
        ...
        private IEnumerable<RawDependency> AnalyzeInstruction([NotNull] TypeDefinition owner, [NotNull] RawUsingItem usingItem,
                                                              ...
                                                              [NotNull] WorkingGraph readingGraph) {
            {
                ...
                // zwischen Nested-Klassen in Archichect.TestAssembly.cs - Klasse Class14.Class13EInner2, Methode SpecialMethodOfInnerClass
                ...
                if (methodReference != null && !IsLinked(methodReference.DeclaringType, owner)) {
                    foreach (var dependency in CreateTypeAndMethodDependencies(usingItem, DOTNETMETHOD,
            ...
            }
            {
                ...
                if (field != null && !IsLinked(field.DeclaringType, owner)) {
                    ...
                    // See comment at Usage._usesmemberoftype on why this is commented out
                    //foreach (var dependency in CreateTypeAndMethodDependencies(usingItem, GetMemberMarkers(field, _fieldDefinitionMarkers), 
            ...
            }
            {
                ...
                if (property != null && !IsLinked(property.DeclaringType, owner)) {
                    foreach (var dependency in CreateTypeAndMethodDependencies(usingItem, DOTNETPROPERTY,
                        GetMemberMarkers(property, _propertyDefinitionMarkers), property.PropertyType,
                        usage: DotNetUsage._usesmember, sequencePoint: sequencePoint, memberName: property.Name, readingGraph: readingGraph)) {
                        yield return dependency;
                    }
                    // See comment at Usage._usesmemberoftype on why this is commented out
                    //foreach (var dependency in CreateTypeAndMethodDependencies(usingItem, GetMemberMarkers(property, _propertyDefinitionMarkers), 
        ...
        private RawUsedItem CreateUsedItem([NotNull] ItemType itemType, [NotNull] TypeReference typeReference, 
            [NotNull] string memberName, [CanBeNull, ItemNotNull] string[] markers, WorkingGraph readingGraph) {
            string namespaceName, className, assemblyName, assemblyVersion, assemblyCulture;
            GetTypeInfo(typeReference, out namespaceName, out className, out assemblyName, out assemblyVersion, out assemblyCulture);
        ...
        private RawUsingItem CreateUsingItem([NotNull] ItemType itemType, [NotNull] TypeReference typeReference,
            [NotNull] string memberName, [CanBeNull, ItemNotNull] string[] markers, [CanBeNull] ItemTail customSections,
            WorkingGraph readingGraph) {
            string namespaceName, className, assemblyName, assemblyVersion, assemblyCulture;
            GetTypeInfo(typeReference, out namespaceName, out className, out assemblyName, out assemblyVersion, out assemblyCulture);
